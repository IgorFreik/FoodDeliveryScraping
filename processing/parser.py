...
from processing.models import MenuItem, MerchantListing
    ...
    if script_tag and script_tag.string:
        try:
            ...
            for key in ("stores", "storeList", "searchResults", "initialData"):
                if key in props:
                    val = props[key]
                    if isinstance(val, list): stores = val
        ...
        if stag and stag.string:
            try:
                ...
                def find_restaurants(obj):
                    if isinstance(obj, dict):
                        if "name" in obj and "location" in obj and ("lat" in obj["location"] or "latitude" in obj["location"]):
                            return [obj]
                        res = []
                        for v in obj.values():
                            found = find_restaurants(v)
                            if found: res.extend(found)
                        return res
                    elif isinstance(obj, list):
                        res = []
                        for i in obj:
                            found = find_restaurants(i)
                            if found: res.extend(found)
                        return res
                    return []
...
def parse_ubereats_listing(html: str, market: str) -> list[MerchantListing]:
    ...
    # RESTORED: Original robust card detection
    containers = soup.select("li[data-testid='carousel-slide'], div[data-testid='store-card'], article, .aj, .ak, .al")
    if not containers:
        containers = soup.select("a[href*='/store/']")

    for container in containers:
        try:
            link_el = container if container.name == "a" else container.select_one("a[href*='/store/']")
            if not link_el: continue
            href = link_el["href"]
            store_id = href.split("?")[0].rstrip("/").split("/")[-1]
            if not store_id or store_id == "store": continue

            name_el = container.select_one("h3, h4, .store-name")
            if not name_el:
                # Try to find name in aria-label of link
                name = link_el.get("aria-label", "")
                if not name: continue
            else:
                name = name_el.get_text(strip=True)

            rating, review_count = None, None
            label_el = container.select_one('[aria-label*="Rating"], [aria-label*="Rated"], [aria-label*="Gewaardeerd"]')
            if label_el:
                label = label_el["aria-label"]
                rmatch = re.search(r"(\d[.,]\d)", label)
                if rmatch: rating = _safe_float(rmatch.group(1).replace(",", "."))
                cmatch = re.search(r"(\d+[\d\s,.]*)\s*(?:reviews|recensies|ratings|waarderingen)", label, re.I)
                if cmatch: review_count = _safe_int(re.sub(r"\D", "", cmatch.group(1)))

            # High-accuracy JSON extraction as supplement
            lat, lng, address = None, None, ""
            found_in_json = False
            for sid in ["__REACT_QUERY_STATE__", "__REDUX_STATE__"]:
                stag = soup.find("script", id=sid)
                if not (stag and stag.string): continue
                ...
                try:
                    ...
                    if raw_text.startswith('"') and raw_text.endswith('"'): raw_text = raw_text[1:-1]
    ...
    if stag and stag.string:
        try:
            ...
            if restaurant:
                ...
                parts = []
                if street and number: parts.append(f"{street} {number}")
