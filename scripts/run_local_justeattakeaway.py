...
async def main():
    print("Starting Amsterdam scraping for JustEatTakeaway...")

    LocalScraper = patch_scraper(JustEatTakeawayListingScraper)
    results = []

    try:
        async with LocalScraper(market="amsterdam", headless=False) as scraper:
            print("Scraping justeattakeaway listings...")
            listings = await scraper.scrape_listings()
            print(f"Found {len(listings)} listings in amsterdam.")

            # Enrich with a few details
            subset = listings[:10]
        ...
        logger.error(f"Failed to run JustEatTakeaway scraper: {e}")
    finally:
        output_file = STORAGE_DIR / "scraped_results_justeattakeaway_amsterdam.json"
