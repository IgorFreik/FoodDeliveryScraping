"""
Airflow DAG: Scrape Uber Eats listings.

Runs weekly (and can be triggered manually). For each configured market,
launches the Uber Eats listing scraper, normalizes results, and loads
...
def _scrape_ubereats(market: str, **kwargs):
    ...
    async def _run():
        async with UberEatsListingScraper(market=market, headless=True) as scraper:
    ...
    logger.info("[DAG] UberEats/%s: %d merchants scraped.", market, len(listings))


# ── DAG Definition ──────────────────────────────────────────────────

with DAG(
    dag_id="scrape_ubereats",
    default_args=default_args,
    description="Scrape Uber Eats restaurant listings per market",
    ...
    for market_config in markets:
        ...
        PythonOperator(
            task_id=f"scrape_ubereats_{slug}",
            python_callable=_scrape_ubereats,
